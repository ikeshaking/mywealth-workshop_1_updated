...
/**
 * Turns an extraction into Mabel's confirmation, in the user's chosen tone.
 * Never judgemental; always offers a next step. The "calm" tone keeps the
 * original wording ("Got it. I've added your …").
 */
export function mabelReply(extraction: Extraction, tone: MabelTone = "calm"): string {
  ...
  if (extraction.is_decision) {
    const byTone: Record<MabelTone, string> = {
  ...
  const openerByTone: Record<MabelTone, string> = {
  ...
  if (extraction.due_date) {
    const tail: Record<MabelTone, string> = {

...
export function buildSeed(today: string): MabelData {
  ...
  const internetRec = recommendFor(internetDecision.request, null, "GBP");
  ...
  const diningRec = recommendFor(diningDecision.request, 2000, "GBP");
  ...
  const paidBill = base({
    ...
    status: "completed",
    priority: "medium",
    context: "£120.00 · Thames Water",
    outcome_summary: "Paid £120.00 to Thames Water — 2 days early.",
